...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, memory,
        ...
        self.nstep = nstep
        #self.noise = [OUNoise(action_size, random.randint(0,2**32)) for _ in range(n_agents)]
        self.memory = memory
        ...
        self.rewards_nstep = [deque(maxlen=nstep) for _ in range(n_agents)]
        self.states_mem = [deque(maxlen=nstep) for _ in range(n_agents)]
        self.actions_mem = [deque(maxlen=nstep) for _ in range(n_agents)]
    ...
    def step(self, states, actions, rewards, next_states, dones):
        ...
        if self.t_ep > self.nstep - 1:
            ...
            states_mem_0 = [self.states_mem[i][0] for i in range(self.n_agents)]
            actions_mem_0 = [self.actions_mem[i][0] for i in range(self.n_agents)]
    ...
    def learn(self, experiences, gamma):
        ...
        noise = (torch.randn_like(actions_next) * self.smooth_policy).clamp(-0.5, 0.5)
